...
#[derive(Debug)]
pub struct AcquireDurationMeasuringPool<DB, M>
where
    DB: Database,
...
impl<'a, DB, M> Acquire<'a> for &'a AcquireDurationMeasuringPool<DB, M>
where
    DB: Database,
    M: AcquireDurationMeter,
{
    ...
    fn acquire(self) -> BoxFuture<'a, Result<Self::Connection, Error>> {
        Box::pin(async move {
            ...
            self.meter.record_acquire_duration(start.elapsed());
            Ok(conn)
        })
    }
...
mod tests {
    ...
    async fn test_acquire() {
        ...
        let pool: AcquireDurationMeasuringPool<_, _> =
        ...
        let tasks: Vec<_> = (0..4)
            ...
            .map(|_| {
                ...
                tokio::spawn(async move {
                    ...
                    sqlx::query(r#"select pg_sleep(0.1)"#)
        ...
        for i in 0..3 {
            ...
            // we should at least measure the pg_sleep(0.1).
            // (measuring actual sleeps is brittle, but using fake time is hard here so let's hope this test doesn't cause headache)
